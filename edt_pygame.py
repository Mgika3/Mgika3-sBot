...
def lundi_de_la_semaine(jour_ref):
    """Renvoie la date du lundi de la semaine contenant jour_ref."""
    return jour_ref - timedelta(days=jour_ref.weekday())

def charger_cours_periode(db_path, debut, fin):
    """Récupère les cours dont la date tombe entre debut et fin (inclus)."""
    try:
    ...
    cours = []
    for nom, jour, heureD, minD, heureF, minF, jour_date, groupe, salle in rows:
...
def charger_cours(db_path=DB_PATH):
    """
    Récupère les cours de la semaine en cours (lundi-dimanche).
    Si aucun cours n'est trouvé (ex: vacances), se replie automatiquement
    sur la semaine suivante. Renvoie (liste_de_cours, lundi, dimanche).
    """
...
def couleur_pour(nom):
    """Couleur stable (toujours la même) associée à un nom de cours."""
...
class Grille:
    ...
    def y_minute(self, minute_depuis_minuit):
        ...
        return self.zone_haut + frac * self.hauteur_grille
...
def dessiner_cours(ecran, grille, cours, police_nom, police_heure, survole):
    ...
    # MODIFIÉ : Afficher nom + groupe sur la 1ère ligne
...
def main():
    ...
    while en_cours:
        ...
        for cours in cours_liste:
            ...
            rect_dessine = dessiner_cours(ecran, grille, cours, police_nom, police_heure, survole)
            rects_dessines.append((rect_dessine, cours))
            if survole:
                bloc_survole = cours
...
def switchPath(licence):
    ...
    if (licence == "L1"):
        ...
        DB_PATH = "database/edtDatabaseL1InfoDiane.db"
    elif (licence == "Sakyna"):
        DB_PATH = "database/edtDatabaseL1InfoSakyna.db"
    elif (licence == "Linda"):
            DB_PATH = "database/edtDatabaseL1InfoLinda.db"
    elif (licence == "Andrea"):
